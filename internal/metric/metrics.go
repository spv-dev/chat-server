...
// DefaultInit инициализация метрики без контекста
func DefaultInit() error {
	ctx := context.Background()
	return Init(ctx)
}

// IncRequestCounter добавление в метрики информации о запросе
func IncRequestCounter() {
	metrics.requestCounter.Inc()
}

// IncResponseCounter добавление в метрики информации об ответе сервера
func IncResponseCounter(status string, method string) {
	metrics.responseCounter.WithLabelValues(status, method).Inc()
}

// HistogramResponseTimeObserve добавление в метрики значение со временем
